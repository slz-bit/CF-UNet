...
parser.add_argument('--config', type=str, default=r'configs\acdc.yaml')
parser.add_argument('--labeled-id-path', type=str,
                    default=r'', help='your labeld data path')
parser.add_argument('--unlabeled-id-path', type=str,
                    default=r'', help='your unlabeld data path')
...
def main():
    ...
    trainset_u = DRIVEDataset(cfg['dataset'], cfg['data_root'], 'train_u',
                             cfg['crop_size'], args.unlabeled_id_path)
    trainset_l = DRIVEDataset(cfg['dataset'], cfg['data_root'], 'train_l',
                             cfg['crop_size'], args.labeled_id_path, nsample=len(trainset_u.ids))
    valset = DRIVEDataset(cfg['dataset'], cfg['data_root'], 'val')

    # trainsampler_l = torch.utils.data.distributed.DistributedSampler(trainset_l)
    trainloader_l = DataLoader(trainset_l, batch_size=cfg['batch_size'],
                               pin_memory=True, num_workers=4, drop_last=True, shuffle=False)
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            loss_u_w_fp1 = criterion_dice(pred_u_w_fp.softmax(dim=1), mask_u_w.unsqueeze(1).float(),
                                          ignore=(conf_u_w < cfg['conf_thresh']).float())

            loss_u_w_fp2 = criterion_dice(pred_u_w.softmax(dim=1), pred_u_w_fp.argmax(dim=1).unsqueeze(1).float(),
                                          ignore=(conf_u_w < cfg['conf_thresh']).float())

            loss_u_w_fp = (loss_u_w_fp1 + loss_u_w_fp2) / 2.0
        ...
        with torch.no_grad():
            for img, mask in valloader:
                ...
                for cls in range(0, cfg['nclass']):
                    ...
                    # print('cls:' + str(cls))
                    dice_class[cls] += 2.0 * inter / union
                    # print('{:}:{:}'.format(cls, dice_class[cls]))
        ...
        if rank == 0:
            ...
            if epoch % 10 == 0:
                torch.save(checkpoint, os.path.join(args.save_path, f'checkpoint-epoch{epoch}.pth'))
